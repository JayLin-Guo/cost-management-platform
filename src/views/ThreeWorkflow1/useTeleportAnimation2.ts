...
import { calculateAnimationSequence, type AnimationSequenceItem, type FlowColorConfig } from './useAnimationSequence'
...
export class TeleportAnimationController2 {
  ...
  private findLabelBetweenNodes(nodeId1: string, nodeId2: string): ConnectionLabelPosition | null {
    ...
    return this.labelPositions.find(l => 
  ...
  public createTeleportNode(): THREE.Mesh {
    ...
    // 使用流程颜色配置或默认颜色
    const teleportNodeColor = initialFlowConfig ? initialFlowConfig.animationColor : COLORS.animation.teleportNode
    const teleportEdgeColor = initialFlowConfig ? initialFlowConfig.edgeColor : COLORS.animation.teleportEdge
    const teleportEmissiveColor = initialFlowConfig ? initialFlowConfig.emissiveColor : COLORS.animation.teleportEmissive
  ...
  private updateAnimation(): void {
    ...
    if (this.teleportTimer >= this.teleportInterval) {
      ...
      // 检查当前位置是否是标签位置
      const isLabelPosition = currentPosInfo ? currentPosInfo.isLabel : this.isLabelPosition(nextPosition)
  ...
  private highlightCurrentElement(): void {
    // 如果没有序列信息，不执行高亮
    if (!this.nodePositionInfos.length || this.currentPositionIndex >= this.nodePositionInfos.length) {
      return
    }
  ...
  private resetHighlight(object: THREE.Object3D): void {
    ...
    if (object instanceof THREE.Mesh) {
      // 如果是网格对象且有材质
      if (object.material instanceof THREE.MeshStandardMaterial && object.userData.originalMaterial) {
        // 恢复原始材质属性
