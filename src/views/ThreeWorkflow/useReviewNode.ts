...
export class ReviewNode {
  ...
  private onStatusChange: ((nodeId: string, newStatus: ReviewNodeStatus, comment?: string) => void) | undefined;
  ...
  private startNodeAnimation(): void {
    ...
    const animateNode = () => {
      if (!this.nodeMesh.parent) return; // 如果节点已被移除，停止动画
          ...
          child.children.forEach((beam, index) => {
            if (beam instanceof THREE.Line) {
              ...
              for (let i = 0; i <= segments; i++) {
                ...
                // 添加动态雷电扰动
                const noise = (Math.sin(progress * Math.PI * 8 + time * 3) + Math.cos(progress * Math.PI * 12 + time * 2)) * 0.8;
                const verticalNoise = Math.sin(progress * Math.PI * 6 + time * 4) * 0.3;
                x += noise;
                z += noise;

                points.push(new THREE.Vector3(x, height + Math.sin(progress * Math.PI * 4 + time) * 0.5 + verticalNoise, z));
              }
  ...
  private addPulseEffect(): void {
    ...
    const animate = () => {
      ...
      // 轻微上下浮动
      this.nodeMesh.position.y =
        this.originalPosition.y + Math.sin(pulseTime) * 0.3;
