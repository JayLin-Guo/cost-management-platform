...
export default function useThreeWorkflow() {
  ...
  let currentPathIndex = 0;        // 当前在路径中的位置索引
  let stayTime = 0;               // 停留时间
  ...
  function initialize(containerElement: HTMLElement, cssContainerElement: HTMLElement) {
    if (!containerElement || !cssContainerElement) return
  ...
  function createExampleReviewNodes(totalWidth: number, calendarDepth: number, cellWidth: number) {
    ...
    reviewers.value.forEach((reviewer, reviewerIndex) => {
      ...
      if (!reviewerData || !reviewerData.timeLine) return;
   ...
   * 创建业务审核流程中节点间的连接关系
   */
  function createBusinessConnections(scene: THREE.Scene, nodes: { [key: string]: {id: string, position: THREE.Vector3} }) {
    // 获取所有节点ID
    ...
    const connectionRules = [
      ...
      // 第二次修订流程路径
      { from: /张三.*初步成果文件\(再修订\)/, to: /李四.*一审送审文件\(再修订\)/, status: '提交一审' },
      { from: /李四.*一审送审文件\(再修订\)/, to: /李四.*一审送审文件\(再修订\)/, status: '审核中' }
    ];
  ...
  function getReviewerStatus(reviewer: any): string {
    ...
    if (reviewerData && reviewerData.timeLine.length > 0) {
      // 按日期排序，找到最新状态
      const latestStatus = [...reviewerData.timeLine]
  ...
  function onWindowResize() {
    if (!camera.value || !renderer.value || !cssRenderer.value) return
  ...
  function onMouseClick(event: MouseEvent) {
    if (!raycaster.value || !mouse.value || !camera.value || !scene.value) return
  ...
  function createFileIcon() {
    if (!scene.value || animationPathPoints.length === 0) return;
    ...
    // 2. 创建外层边框 - 与审核节点相同
    const outerEdgeGeometry = new THREE.EdgesGeometry(new THREE.BoxGeometry(boxWidth + 4, 8, boxHeight + 4));
  ...
  function createDocumentIcon(): THREE.Group {
    ...
    const line1Vertices = new Float32Array([
      -4, 0, 3,
      4, 0, 3
    ]);
    ...
    const line2Vertices = new Float32Array([
      -4, 0, 0,
      4, 0, 0
    ]);
    ...
    const line3Vertices = new Float32Array([
      -4, 0, -3,
      4, 0, -3
    ]);
  ...
  function startSimpleGlowAnimation(fileGroup: THREE.Group) {
    ...
    const animateGlow = () => {
      if (!fileGroup.parent) return; // 如果文件组已被移除，停止动画
      ...
      fileGroup.traverse((child) => {
        if (child.userData.isGlow && child instanceof THREE.Mesh) {
          ...
          child.material.opacity = 0.10 + Math.sin(elapsed * 0.8) * 0.02; // 减少透明度变化
        }
      });
  ...
  function createDetailedPath() {
    if (!scene.value || animationPathPoints.length < 2) return;
  ...
  function updateFileAnimation(delta: number) {
    if (!isAnimationPlaying || !fileIcon || pathPoints.length < 2) return;
  ...
  function updateFileStatusLabel(status: string) {
    if (!fileIcon || !scene.value) return;
   ...
   */
  function animate() {
    if (!scene.value || !camera.value || !renderer.value || !cssRenderer.value) return
  ...
  function startFlowAnimation() {
    if (animationPathPoints.length < 2) return;
